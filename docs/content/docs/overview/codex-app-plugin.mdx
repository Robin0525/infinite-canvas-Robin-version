...
```text
Install the Infinite Canvas Codex plugin from https://github.com/Robin0525/infinite-canvas-Robin-version.git.
Clone it to ~/plugins/infinite-canvas-Robin-version, verify plugins/infinite-canvas/.codex-plugin/plugin.json,
...
```bash
mkdir -p ~/plugins
git clone https://github.com/Robin0525/infinite-canvas-Robin-version.git ~/plugins/infinite-canvas-Robin-version
codex plugin marketplace add ~
codex plugin add infinite-canvas@personal
```

The personal marketplace entry must point to `./plugins/infinite-canvas-Robin-version/plugins/infinite-canvas`. Start a new Codex conversation after installation so the Skill and MCP tools load completely.
