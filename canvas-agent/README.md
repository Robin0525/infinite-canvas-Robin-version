...
```bash
npx -y github:Robin0525/infinite-canvas-Robin-version#main --debug
```
...
直接运行 `npx -y github:Robin0525/infinite-canvas-Robin-version#main` 只启动本地 Agent 服务，不会安装 MCP，也不会增加 Codex 工具上下文。只有安装 Codex app 插件，或手动执行 `codex mcp add` 后，`infinite-canvas` 工具才会进入 Codex 上下文；由于工具较多，不使用时建议移除。
...
```bash
npx -y github:Robin0525/infinite-canvas-Robin-version#main mcp
```
...
```bash
codex mcp add infinite-canvas -- npx -y github:Robin0525/infinite-canvas-Robin-version#main mcp
```
...
command = "npx"
args = ["-y", "github:Robin0525/infinite-canvas-Robin-version#main", "mcp"]
...
```bash
claude mcp add --scope user --transport stdio infinite-canvas -- npx -y github:Robin0525/infinite-canvas-Robin-version#main mcp
```
