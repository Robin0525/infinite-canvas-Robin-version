...
- [新增] 画布库增加嵌套文件夹管理，支持新建、重命名、复制、移动和递归删除文件夹；画布支持“移动到”，原“复制”升级为可选择目标路径的“复制到”，并在 WebDAV 画布数据中同步保存文件夹结构。
- [修复] 中英文切换时左上角品牌标题同步显示“无限画布”或“Infinite Canvas”；桌面端 GitHub 图标改用系统浏览器打开 Robin Version 主仓库，避免 Tauri 内嵌窗口拦截新标签页。
...
- [修复] 点击版本号后的更新记录弹窗改为读取 Robin Version 新仓库的 Releases，并兼容标准 Markdown 列表解析内置 CHANGELOG；网络读取失败时仍可显示完整更新记录，右上角 GitHub 入口与插件安装说明同步指向新仓库。
- [调整] 生成配置开启批量处理后，“生成张数”改为默认 1 次的“生成次数”；每次按元素组笛卡尔积生成独立的一组结果，多次执行按组纵向排列，不再扁平合并或额外复制首张结果。
- [优化] 批量并发改为显式最多同时提交 10 个独立组合，依次模式仍严格等待上一项完成后再提交下一项；同一配置再次点击生成时，新结果自动排列到已有结果下方。
- [修复] 删除元素组成员后立即清理成员顺序并重新计算网格布局，无需再手动缩放元素组触发刷新。

- [新增] Infinite Canvas Codex 插件与 Canvas Agent 支持创建元素组、设置和重排组成员，并可连接元素组配置并发或依次批量生成。
- [修复] 含视频内容的节点在播放器获得焦点后仍可使用 `Delete` 或 `Backspace` 删除，不再被视频控件的防画布交互标记拦截。
- [新增] 批量生成在提交前弹出确认窗口，明确展示元素组循环规则、组合数量、单组合生成数、总结果数及并发/依次执行方式；取消确认时不创建任务。
- [修复] 批量组合会排除已属于元素组但仍残留独立连线的成员，固定图片只作为每个组合的公共输入，元素组中的图片、视频或文字则按当前组合逐项注入，避免多张提示词被拼成一张结果。
- [优化] 依次模式严格等待上一项完整生成并保存后再冷却提交下一项；并发模式限制为最多 5 个独立组合，图片接口每次只请求 1 张结果。批量请求遇到 429、502、503 时自动退避重试 2 次，并补充可直接定位中转站或上游服务异常的中文说明。
- [新增] 左下角重复的快捷键问号入口替换为“自动整理画布”：单选时整理相连的完整上下游工作流，多选时仅整理所选节点；按拓扑从左到右分层，组和批量结果作为整体移动，并支持现有撤销操作。
- [修复] 向空图片或视频节点导入、替换本地内容，以及生成结果写回空节点时保留节点原有宽高与位置，避免大尺寸素材使节点突然铺满画布。
