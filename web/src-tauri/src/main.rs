...
fn save_download(app: tauri::AppHandle, filename: String, bytes: Vec<u8>) -> Result<bool, String> {
    ...
    std::fs::write(
        path.as_path()
            .ok_or("The selected path is not a local file")?,
        bytes,
    )
...
async fn google_oauth_authorize(
    ...
    let (code, mut browser_stream) =
        tauri::async_runtime::spawn_blocking(move || -> Result<(String, TcpStream), String> {
            ...
            loop {
                match listener.accept() {
                    Ok((mut stream, _)) => {
                        ...
                        let result = if params.get("state").map(|value| value.as_ref())
                            != Some(expected_state.as_str())
                        {
                            ...
                            params
                                ...
                                .ok_or_else(|| "Google 授权没有返回登录凭证".to_string())
                        };
                        return match result {
                    ...
                    Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => {
                        return Err("等待 Google 登录超时，请重新连接".to_string())
                    }
    ...
    let token_result = async {
        ...
        let access_token = token.access_token.ok_or_else(|| {
            token
                .error_description
                .or(token.error)
                .unwrap_or_else(|| "Google 未返回访问令牌".to_string())
        })?;
        let refresh_token = token
            .refresh_token
            .ok_or_else(|| "Google 未返回刷新令牌，请撤销应用授权后重新连接".to_string())?;
        Ok::<(String, u64, String), String>((
            access_token,
...
async fn google_oauth_refresh(
    ...
    let access_token = token.access_token.ok_or_else(|| {
        token
            .error_description
            .or(token.error)
            .unwrap_or_else(|| "Google 刷新登录状态失败，请重新连接".to_string())
    })?;
...
#[tauri::command]
fn install_update(app: tauri::AppHandle, filename: String, bytes: Vec<u8>) -> Result<(), String> {
    let safe_name = std::path::Path::new(&filename)
        .file_name()
        .and_then(|value| value.to_str())
        .ok_or("Invalid update filename")?;
    if bytes.is_empty() || !safe_name.to_ascii_lowercase().ends_with(".exe") {
        return Err("Invalid Windows update installer".to_string());
    }
    let update_dir = std::env::temp_dir()
        .join("InfiniteCanvasRobinVersion")
        .join("updates");
    std::fs::create_dir_all(&update_dir).map_err(|error| error.to_string())?;
    let installer_path = update_dir.join(safe_name);
