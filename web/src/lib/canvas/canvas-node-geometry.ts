...
export function findGroupAtPosition(position: Position, movedIds: Set<string>, nodes: CanvasNodeData[]) {
    if (nodes.some((node) => movedIds.has(node.id) && node.type === CanvasNodeType.Group)) return null;
    return (
        [...nodes]
            .reverse()
            .find(
                (group) => group.type === CanvasNodeType.Group && !movedIds.has(group.id) && position.x >= group.position.x && position.x <= group.position.x + group.width && position.y >= group.position.y && position.y <= group.position.y + group.height,
            ) || null
    );
}
