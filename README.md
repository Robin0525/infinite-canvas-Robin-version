...
- 画布库新增嵌套文件夹，新建、重命名、复制、移动、删除及画布“移动到/复制到”均支持目标路径选择。
- 多张和批量图片生成恢复独立总览图，最多自动拼成九宫格；实际结果仍保持独立节点并可双击放大。
- Windows 安装包携带 Robin Version Codex 插件市场快照，插件和 Canvas Agent 安装入口统一指向 Robin0525 主仓库。

### v0.15.3

- 批量生成前显示循环组合、单组合生成数、总任务数和执行方式，确认后才正式提交任务。
- 左下角增加自动整理画布，可按上下游拓扑整理整个工作流，也可只整理多选节点；元素组与批量结果保持整体移动。
- 批量任务支持瞬时网关错误自动退避重试，并进一步区分固定素材与元素组的逐项组合输入。
